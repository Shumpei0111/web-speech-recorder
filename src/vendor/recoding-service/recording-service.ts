...
interface IRecordingService {
  ...
  micGainNode: GainNode | undefined | null;
  outputGainNode: GainNode | undefined | null;
  dynamicsCompressorNode: DynamicsCompressorNode | undefined | null;
  analyserNode: AnalyserNode | undefined | null;
  // TODO: この機能は AudioWorklet と AudioWorkletNode インターフェイスに置き換え
  // https://developer.mozilla.org/ja/docs/Web/API/BaseAudioContext/createScriptProcessor
  processorNode: ScriptProcessorNode | undefined | null;
  destinationNode:
    | MediaStreamAudioDestinationNode
    | AudioDestinationNode
    | undefined
    | null;
  encoderWorker: Worker | undefined | null;
  micAudioStream: MediaStream | undefined | null;
  inputStreamNode: MediaStreamAudioSourceNode | undefined | null;
  baseAudioCtx: BaseAudioContext | undefined;
  ...
  startRecording(timeSlice: any): Promise<void> | undefined;
  setMicGain(newMicGain: any): void;
  _startRecordingWithStream(stream: MediaStream, timeSlice: any): void;
  _onAudioProcess(e: any): void;
  stopRecording(): void;
  _onDataAvailable(evt: any): void;
...
export class RecorderService implements IRecordingService {
  baseUrl: string;
  em: DocumentFragment;
  state: string;
  chunks: any[];
  ...
  startRecording(timeslice: any): Promise<void> | undefined {
    ...
    if (!this.config.usingMediaRecorder) {
      ...
      this.encoderWorker.addEventListener("message", (e) => {
        ...
        // _onDataAvailable にカスタムイベントを渡す
        this._onDataAvailable(customEvent);
      });
    }
  ...
  _startRecordingWithStream(stream: MediaStream, timeslice: number) {
    ...
    this.baseAudioCtx = this.inputStreamNode?.context;
    ...
    if (this.micGainNode) {
      ...
      this.micGainNode.gain.setValueAtTime(
        this.config.micGain,
        this.baseAudioCtx?.currentTime || 0
      );
    }

    let nextNode: GainNode | null | undefined | DynamicsCompressorNode =
      this.micGainNode;
    ...
    if (this.processorNode) {
      ...
      this.outputGainNode && this.processorNode.connect(this.outputGainNode);
      this.processorNode.onaudioprocess = (e) => this._onAudioProcess(e);
    } else {
      this.outputGainNode && nextNode?.connect(this.outputGainNode);
    }
    ...
    this.destinationNode && this.outputGainNode?.connect(this.destinationNode);

    if (this.config.usingMediaRecorder) {
      if (!(this.destinationNode instanceof MediaStreamAudioDestinationNode))
        return;
  ...
  _onDataAvailable(evt: any): void {
    ...
    if (typeof this.chunkType !== "string") return;
